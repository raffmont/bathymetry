...
- Tile outputs (generated):
  - `./out/bathy_contours.mbtiles` (vector, required)
  - `./out/bathy_raster.mbtiles` (optional)
  - `./out/bathy_hillshade.mbtiles` (optional)
  - `./out/bathy_slope.mbtiles` (optional)
  - `./out/tilejson/*.tilejson` (optional, for integration)
...
```bash
python obfuscate_bathy_tiles.py config.all_features.json --no-obfuscation
```
